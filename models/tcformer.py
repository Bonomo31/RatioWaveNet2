...
class TCFormerModule_ablation(nn.Module):
    def __init__(self,                 
        ...
        # # ablation studies, MK-CNN + TCN 
        # # self.tcn_head = TCNHead(d_group*(self.n_groups)*2, 2, tcn_depth, 
        ...
        # # for ablation studies,  MK-CNN and MK-CNN + Trans
        # self.classifier = ClassificationHead( 
        #     d_features=d_group*(self.n_groups), # d_group*(self.n_groups+1) for MK-CNN + Trans
        #     n_groups=self.n_groups, # (self.n_groups+1) for MK-CNN + Trans
        #     n_classes=n_classes,
        #     kernel_size=17,  # point-wise conv
        # )   
    def forward(self, x):      # x: [B, C_electrodes, T]
        # # CNN only ---------------------------------
        # conv_features = self.conv_block(x)         
        # out = self.classifier(conv_features)        # for ablation studies, MK-CNN only
        ...
        # out = self.classifier(features)        # for ablation studies, MK-CNN + Transformer
        # # out = self.classifier(self.rearrange(tran_features))        # for ablation studies, MK-CNN + Transformer
