"""
Model Name: RatioWaveNet

Description:
This model is inspired by the TCFormer architecture. It extends the original 
design by integrating a Graph Neural Network (GNN) front-end to explicitly 
model spatial dependencies between EEG channels, enhancing the baseline 
Temporal Convolutional Transformer structure.

Base Architecture Reference:
Adapted from TCFormer: H. Altaheri, F. Karray, and A.H. Karimi (2025). 
Original repository: https://github.com/altaheri/TCFormer
"""
...
class EEGGraphLayer(nn.Module):
    """
    GNN front-end stile GATE per RatioWaveNet.

    - Ogni canale EEG è un nodo.
    - Usa una matrice di adiacenza A (fisica o pseudo-fisica),
      normalizzata A_hat = D^{-1/2}(A+I)D^{-1/2}.
    - Applica Chebyshev-GCN di ordine K-1 (default K=3).
    - Applica channel attention stile SE.
    - Residual verso l'input con alpha inizializzato a 0 (identità all'inizio).

    Input:
        x: (B, C, T) oppure (B, 1, C, T)
    Output:
        stessa shape dell'input.
    """
    def __init__(
        self,
        num_channels: int,
        ...
        # --- 1) costruzione A di base ---
        if adjacency is None:
            # fallback: grafo "a catena" (ring) molto semplice
            A = torch.eye(num_channels)
            for i in range(num_channels):
                A[i, (i - 1) % num_channels] = 1.0
                A[i, (i + 1) % num_channels] = 1.0
        else:
            A = adjacency.clone().float()
            assert A.shape == (num_channels, num_channels), \
                f"Adjacency must be ({num_channels},{num_channels}), got {A.shape}"

        if symmetric:
            A = 0.5 * (A + A.t())

        # aggiungi self-loops
        I = torch.eye(num_channels)
        A_tilde = A + I
        # degree matrix
        D_tilde = torch.diag(A_tilde.sum(dim=-1))
        # normalizzazione simmetrica
        D_inv_sqrt = torch.linalg.inv(torch.sqrt(D_tilde))
        A_hat = D_inv_sqrt @ A_tilde @ D_inv_sqrt

        # registra A_hat e polinomi di Chebyshev T_k(A_hat) come buffer
        self.register_buffer("A_hat", A_hat)
        T_list = []
        T0 = torch.eye(num_channels)
        T1 = A_hat.clone()
        T_list.append(T0)
        if K > 1:
            T_list.append(T1)
        for k in range(2, K):
            Tk = 2 * A_hat @ T_list[-1] - T_list[-2]
            T_list.append(Tk)
        self.register_buffer("T_cheb", torch.stack(T_list, dim=0))  # (K, C, C)

        # --- 2) GCN weight matrix (C -> C) ---
        self.W_gc = nn.Linear(num_channels, num_channels, bias=False)

        # --- 3) Channel attention stile SE ---
        ...
        # residual scaling
        if use_residual:
            # partiamo da 0 -> identità iniziale
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.dim() == 4:
            ...
            raise ValueError(
                f"EEGGraphLayer expects input of shape (B,C,T) or (B,1,C,T), got {x.shape}"
            )

        B, C, T = x.shape
        # (B, C, T) -> (B*T, C)
        x_flat = x.permute(0, 2, 1).contiguous().view(B * T, C)

        # Chebyshev GCN: sum_k T_k(A_hat) @ x
        # T_cheb: (K, C, C)
        T_cheb = self.T_cheb.to(x.device)   # (K, C, C)
        z_flat = torch.zeros_like(x_flat)
        for k in range(self.K):
            Tk = T_cheb[k]                  # (C, C)
            z_flat = z_flat + x_flat @ Tk.t()

        # linear mixing (C -> C)
        z_flat = self.W_gc(z_flat)
        z_flat = self.dropout(z_flat)

        # reshape back to (B, C, T)
        z = z_flat.view(B, T, C).permute(0, 2, 1).contiguous()  # (B, C, T)

        # channel SE attention
        w = self.se(z)        # (B, C, 1), tra 0 e 1
        z_att = z * w

        if self.use_residual:
            # residual identity-friendly: start as pure x, poi impara ad usare la GNN
            out = x + self.alpha * (z_att - x)
        else:
            out = z_att

        if len(original_shape) == 4:
            out = out.unsqueeze(1)  # (B, 1, C, T)
...
# ------------------------------------------------------------------------------- #
class RatioWaveNetModule(nn.Module):
    def __init__(self,                 
            n_channels: int ,
            n_classes: int,
            F1: int = 16,
            ...
            trans_depth: int = 5,
            use_gnn: bool = True,          # <<< NEW
            gnn_dropout: float = 0.1,      # <<< NEW
        ):
        super().__init__()
        self.n_classes = n_classes
        ...
        # --- Primo layer: GNN sui canali EEG (opzionale) ---
        self.use_gnn = use_gnn
        if self.use_gnn:
            self.gnn = EEGGraphLayer(
